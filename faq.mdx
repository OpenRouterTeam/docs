...
<AccordionGroup>
  ...
  <Accordion title='Why do I see "You must add a verified email to access this feature"?'>
    Certain actions — such as purchasing credits or creating an organization — require a verified email address on your account.

    To verify your email:

    1. Open [Settings > Account](https://openrouter.ai/settings/account).
    2. Add an email address if none is set, then click **Send verification email**.
    3. Open the verification link in the email we send you (check your spam folder if it doesn't arrive within a few minutes).

    Once verified, return to the page you were trying to use and retry the action. If the email never arrives, contact [support@openrouter.ai](mailto:support@openrouter.ai).
  </Accordion>
