...
<AccordionGroup>
  ...
  <Accordion title='What are model variants?'>
    Variants are suffixes that can be added to the model slug to change its behavior. There are two kinds. See the [Model Variants overview](/guides/routing/model-variants/overview) for the full reference and the rule for resolving a suffixed ID to its metadata.
    ...
    1. `:free` - A free version of the model with its own rate limits. [Learn more](/guides/routing/model-variants/free).
    2. `:batch` - The batch-priced version of the model used by the [Batch API](/batch-quickstart).
    ...
    1. `:nitro` - Providers will be sorted by throughput rather than the default sort, optimizing for faster response times. [Learn more](/guides/routing/model-variants/nitro).
    2. `:floor` - Providers will be sorted by price rather than the default sort, prioritizing the most cost-effective options. [Learn more](/guides/routing/model-variants/floor).
    3. `:exacto` - Providers will be sorted using quality-first signals tuned for tool-calling reliability. [Learn more](/guides/routing/model-variants/exacto).
    4. `:online` (deprecated) - All requests will run a query to extract web results that are attached to the prompt. Use the [`openrouter:web_search` server tool](/guides/features/server-tools/web-search) instead. [Learn more](/guides/routing/model-variants/online).
