---
title: Logs
description: Inspect individual generations, upstream requests, sessions, and async jobs from your OpenRouter dashboard
seoTitle: 'Logs | OpenRouter Observability'
og:site_name: OpenRouter Documentation
og:title: 'Logs - Inspect Every OpenRouter Generation'
og:description: 'Use the Logs page to inspect each generation, its routing timeline, token usage, cost, and stored prompt and completion content.'
og:image: https://openrouter.ai/dynamic-og?title=Logs&description=Inspect%20every%20OpenRouter%20generation
og:image:width: 1200
og:image:height: 630
...
<Frame>
![The Logs page showing the Generations tab with a table of recent generations](/assets/features/logs/generations.png "Logs page, Generations tab")
</Frame>
...
| **Batches** | [Batch API](/batch-quickstart) jobs and their status. |
...
<Frame>
![The generation detail sheet showing latency, cost, token counts, request metadata, and the provider response waterfall](/assets/features/logs/generation-details.png "Generation detail sheet")
</Frame>
...
- **Prompt** and **Completion**: the full request and response content when [Input & Output Logging](/guides/features/input-output-logging) is enabled.
...
By default OpenRouter stores only metadata about each generation. To keep the prompt and completion text as well, enable [Input & Output Logging](/guides/features/input-output-logging) under **Settings > Observability**. Only generations made after you enable it will have stored content.
...
| Endpoint | Returns |
|----------|---------|
| [`GET /api/v1/generation`](/api/api-reference/generations/get-request-&-usage-metadata-for-a-generation) | The request and usage metadata shown in the detail sheet: model, provider, tokens, cost, latency, and finish reason. |
| [`GET /api/v1/generation/content`](/api/api-reference/generations/get-stored-prompt,-completion,-and-error-content-for-a-generation) | The stored prompt, completion, and error content. Requires [Input & Output Logging](/guides/features/input-output-logging) to have been enabled when the generation was made. |

## Related

- [Broadcast](/guides/features/broadcast): send traces for every generation to your own observability platform.
- [Activity Export](/cookbook/administration/activity-export): download aggregated usage as CSV or PDF.
