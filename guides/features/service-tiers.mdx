...
See [Nitro](/guides/routing/model-variants/nitro) and [Floor](/guides/routing/model-variants/floor) for the variants in full.
...
- On models that support fast mode, the request routes to the model's `fast` service tier endpoint like any other provider's priority tier. See [Fast Mode](/cookbook/coding-agents/claude-code-integration#fast-mode) in the Claude Code guide.
...
- [`:nitro`](/guides/routing/model-variants/nitro) makes priority endpoints eligible. [`:floor`](/guides/routing/model-variants/floor) makes flex endpoints eligible.
...
- Combine with [`allow_fallbacks: false`](/guides/routing/provider-selection#disabling-fallbacks) to route only to the top endpoint of that tier.
...
- `"provider": { "only": ["openai/fast"] }` restricts routing to OpenAI's Fast tier. See [Provider Selection](/guides/routing/provider-selection) for `provider.order` and `provider.only` in full.
