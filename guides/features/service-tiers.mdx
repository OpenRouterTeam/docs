...
1. **The `service_tier` parameter.** Endpoints matching the requested tier are tried first (`priority` sorted by throughput, `flex` by price), with fallback to other endpoints if none succeed. Billing always follows the endpoint actually used, so a request that falls back off-tier is charged at that endpoint's standard rate, not the tier rate. Combine with [`allow_fallbacks: false`](/docs/features/provider-routing#disabling-fallbacks) to route only to the top endpoint of that tier.
