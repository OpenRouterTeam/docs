...
description: Keep prompts and completions inside the EU or the US
seoTitle: 'In-Region Routing | Keep AI Requests Inside the EU or the US'
og:site_name: OpenRouter Documentation
og:title: 'In-Region Routing - Keep AI Requests Inside the EU or the US'
og:description: 'Learn how OpenRouter in-region routing keeps prompts and completions inside the EU or the US, which features are available, and how BYOK works with regional routing.'
og:image: https://openrouter.ai/dynamic-og?title=In-Region%20Routing&description=Keep%20AI%20requests%20inside%20the%20EU%20or%20the%20US
og:image:width: 1200
og:image:height: 630
...
- The request routes through your prioritized BYOK key for a provider whose only endpoints for the model are regional, billing the premium to your own provider account. See [BYOK and Regional Endpoints](/guides/overview/auth/byok#byok-and-regional-endpoints)
...
Sending requests to a regional base URL is a per-request choice. To guarantee that traffic for a key, member, or an entire workspace can only enter OpenRouter through a specific region, restrict the allowed data regions in a [guardrail](/guides/features/guardrails). Each guardrail can specify `allowed_data_regions`, a list of the OpenRouter domains that requests governed by that guardrail must arrive through:
...
To enforce EU residency for a whole workspace, set `allowed_data_regions` to `["europe"]` on the [workspace default guardrail](/guides/features/guardrails#updating-the-workspace-default-guardrail-via-api). This can be configured in the guardrail editor under **Data regions** or through the [Guardrails API](/api/api-reference/guardrails/create-a-guardrail).

<Note>
The guardrail value for the EU is `europe`, while the [Models API](/api/api-reference/models/list-all-models-and-their-properties) region filter takes `region=eu`. The two are not interchangeable.
</Note>
...
- **Web search, except Exa in the US.** The [web search plugin](/guides/features/plugins/web-search) only runs on a regional domain through an engine whose backend is resident in that region. Today that is Exa on `us.openrouter.ai`: Exa serves US requests from in-region indexes, so the query never leaves the region. `engine: "exa"`, `engine: "auto"`, and an omitted engine all run on Exa there; `auto` and an omitted engine skip the provider's native search on regional domains. Every other configuration is rejected. Perplexity, Parallel, and Firecrawl process queries outside your data region, and `engine: "native"` is not covered by the provider's regional inference commitment. No web search plugin engine is available on `eu.openrouter.ai`.
- **Web search and web fetch server tools.** The [`web_search`](/guides/features/server-tools/web-search) server tool follows the same rule as the plugin: `engine: "exa"`, `engine: "auto"`, and an omitted engine run on Exa on `us.openrouter.ai`, and every other configuration is rejected on regional domains. The [`web_fetch`](/guides/features/server-tools/web-fetch) server tool is backed by out-of-region fetch vendors, so it is not available on regional domains.
- **Other server tools with out-of-region infrastructure**, including [files](/guides/features/server-tools/files), [image generation](/guides/features/server-tools/image-generation), [Fusion](/guides/features/server-tools/fusion), and [shell](/guides/features/server-tools/shell). Server tools that execute entirely in-region (such as [tool search](/guides/features/server-tools/tool-search), [apply patch](/guides/features/server-tools/apply-patch), and [datetime](/guides/features/server-tools/datetime)) remain available.
- **Batch API.** The [Batch API](/batch-quickstart) currently runs on US-based storage and queueing infrastructure and does not yet support regional data residency. Submit batch requests through the global endpoint at `openrouter.ai`.
- **Multi-model routers** such as the Auto Router, as described above.

Zero Data Retention ([ZDR](/guides/features/zdr)) enforcement and [data collection controls](/guides/privacy/provider-logging) work normally with in-region routing and are commonly combined with it. See [Sovereign AI](/guides/features/sovereign-ai) for how these features compose.

## Observability with In-Region Routing

[Broadcast](/guides/features/broadcast) supports in-region routing. Every broadcast destination is configured with the data regions it receives traces from (global, EU, or US), and a trace from a regional request is only delivered to destinations configured for that region.
...
Separately, OpenRouter's own [input/output logging](/guides/features/input-output-logging) is currently skipped for all requests through the regional endpoints, even when the feature is enabled. Requests work as normal; prompt and completion logging is simply not performed.

## BYOK with In-Region Routing

[BYOK](/guides/overview/auth/byok) works with in-region routing, but there is an important boundary to understand:
...
Your key is only used for a model when the provider's shared endpoint for that model is itself eligible for the region. When that shared endpoint is outside your data region but your own account with the provider processes requests inside it, declare the region on the key under **Provider agreement → Data region** in your workspace BYOK settings. See [Declaring a Data Region on a Key](/guides/overview/auth/byok#declaring-a-data-region-on-a-key) for the options and the cases where the declaration is not honored.
...
To use Baseten under in-region routing, create a [regional Dedicated Deployment](https://docs.baseten.co/deployment/regional-deployments) in Baseten and add its URL as a [private model](/guides/routing/private-models) on OpenRouter. OpenRouter derives the region from the `model-{model_id}-region-{us|eu}.api.baseten.co` hostname: a `region-eu` deployment is eligible on `eu.openrouter.ai`, a `region-us` deployment on `us.openrouter.ai`.
...
- Review [Sovereign AI](/guides/features/sovereign-ai) for combining in-region routing with ZDR and data collection controls
- Configure [BYOK](/guides/overview/auth/byok) keys with region-appropriate deployments as described above
