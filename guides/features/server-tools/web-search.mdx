...
In both modes a request with no `engine` (or `engine: "auto"`) runs the first engine in the workspace list (see the `native` row under [Engine Selection](#engine-selection) for the native fallback). Settings on the workspace's Plugins tab do not apply to server tools.
