...
---
title: Shell
description: Give any model a sandboxed hosted shell on the Responses API
...
og:description: "Add a hosted shell to any model on OpenRouter's Responses API. A sandbox-backed clone of OpenAI's hosted shell tool — the model runs commands server-side in an isolated container."
...
The shell server tool is only available through the [Responses API](/api_reference/responses). Requesting it on the Chat Completions or Messages API returns a `400` error. On the Messages API, use [Bash](/guides/features/server-tools/bash) instead.
</Warning>
