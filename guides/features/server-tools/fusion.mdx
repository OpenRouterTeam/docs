...
This is the same pipeline behind the [`openrouter/fusion` model alias](/guides/routing/routers/fusion-router) and the [`fusion` plugin](/guides/features/plugins/fusion). Using the server tool directly gives you the most control: choose your own outer model, combine it with other tools, and configure the panel and analyst independently.
