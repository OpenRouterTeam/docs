...
Fusion in the `tools` array works on `/chat/completions` today, but it's slower there than on the [Responses API](/api_reference/responses). For latency-sensitive use, send the same `tools: [{ type: "openrouter:fusion" }]` payload to the Responses API instead.
</Note>
