...
`openrouter:bash` is only available on the Anthropic Messages API, on every
`engine`. Requesting it on the Chat Completions or Responses API returns a
`400` error.

For **sandboxed** commands on those APIs, use
[Shell](/guides/features/server-tools/shell) — it runs server-side in the same
