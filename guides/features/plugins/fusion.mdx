---
title: Fusion
description: Multi-model analysis with an analyst model
seoTitle: 'Fusion | Multi-model AI Analysis with OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Fusion - Multi-model Analysis on OpenRouter'
og:description: "Get richer answers by running a panel of models in parallel and comparing their responses through an analyst. Fusion makes this a one-line addition to any OpenRouter request."
og:image: https://openrouter.ai/dynamic-og?title=Fusion&description=Multi-model%20Analysis%20with%20a%20Analyst%20Model
og:image:width: 1200
og:image:height: 630
...
|-------|---------|-------------|
| `preset` | _none_ | A curated OpenRouter preset slug (e.g. `general-high`) that expands into a panel + analyst, so you don't have to name models. Explicit `analysis_models` / `model` override it. See [Presets](#presets). |
