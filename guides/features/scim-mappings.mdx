...
- Your organization must have an active SSO connection (see [Single Sign-On (SSO)](/guides/features/sso))
...
<AccordionGroup>
  ...
  <Accordion title="Why is my group list empty?">
    Groups appear once your identity provider pushes them via SCIM provisioning. Check that provisioning is [set up](#set-up-provisioning) and that your IdP's provisioning configuration pushes groups, then click **Refresh from IdP**. You also need an active [SSO connection](/guides/features/sso).
  </Accordion>
