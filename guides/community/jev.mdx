---
title: 'Jev Documentation: Using the TypeSafe Decision Model on OpenRouter'
sidebarTitle: Jev (TypeSafe)
description: The OpenRouter hub for Jev, the TypeSafe decision model. What Jev is, how to get access, the Decisions API, TypeScript and Python SDKs, cookbooks, pricing, and demos
seoTitle: 'Jev Documentation | TypeSafe Decision Model on OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Jev Documentation - TypeSafe Decision Model on OpenRouter'
og:description: 'The OpenRouter hub for Jev, the TypeSafe System One decision model. Access, quickstart, Decisions API reference, TypeScript and Python SDKs, cookbooks, pricing, and Jev Lab demos.'
og:image: https://openrouter.ai/dynamic-og?title=Jev%20Documentation&description=Use%20the%20TypeSafe%20decision%20model%20on%20OpenRouter
og:image:width: 1200
og:image:height: 630
...
<CardGroup cols={2}>
  <Card title="Jev Tutorial" icon="bolt" href="/guides/community/jev-tutorial">
  ...
  <Card title="Decisions API reference" icon="code" href="/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request">
    Full request and response schema for `POST /api/alpha/decisions`.
  </Card>
  <Card title="Jev SDK for TypeScript and Python" icon="puzzle-piece" href="/guides/community/typesafe-sdk">
...
There's no waitlist or separate TypeSafe account. Just create an [OpenRouter API key](https://openrouter.ai/settings/keys), and then follow the [Jev tutorial](/guides/community/jev-tutorial). Your first call takes under five minutes.
...
| --- | --- | --- |
| Decisions API | `POST https://openrouter.ai/api/alpha/decisions` | You are calling Jev from any language with plain HTTP, or through the OpenRouter [TypeScript](/client-sdks/typescript/sdks/decisions/README), [Python](/client-sdks/python/sdks/decisions/README), or [Go](/client-sdks/go/sdks/decisions/README) SDK |
...
The [Jev SDK guide](/guides/community/typesafe-sdk) documents the System One path. The [Decisions API reference](/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request) covers the Decisions path.

### Cookbooks

Step-by-step recipes with runnable code:

- [Gate Agent Tool Calls with Jev](/cookbook/building-agents/gate-tool-calls-with-jev) checks each risky agent tool call against the user's request so safe calls run, unsupported calls are refused, and only ambiguous calls pause for a human.
- [Cut LLM Cost with a Jev-Verified Cascade](/cookbook/evaluate-and-optimize/jev-verified-cascade) drafts with a cheap model, verifies the draft against retrieved context with Jev, and escalates to a stronger model only when the check fails.
- [Classify and Tag Text at Scale with Jev](/cookbook/evaluate-and-optimize/jev-classification) labels each item in a batch with one category and any number of tags, picks a threshold per tag from a small labeled sample, and computes cost per 1,000 items from `usage.cost`.
- [Classify Reddit and YouTube Comments with Jev and ScrapeCreators](/cookbook/evaluate-and-optimize/classify-reddit-comments-with-jev) scrapes comments, scores each one for relevance and sentiment, and stores the raw judgments so you can re-tune thresholds without new requests.
- [Auto-Approve Coding Agent Permission Prompts with Jev](/cookbook/coding-agents/auto-approve-permission-prompts-with-jev) answers the permission prompts from Claude Code, Codex, Cursor, and OpenCode with a Jev reversibility check, so routine commands run and risky ones still ask.
