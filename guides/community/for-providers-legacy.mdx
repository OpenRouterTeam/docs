---
title: Provider Integration (Legacy Format)
seoTitle: 'Provider Integration Legacy Format | OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Provider Integration (Legacy Format) - OpenRouter'
og:description: "Reference for OpenRouter's legacy flat provider model document format, kept for existing integrations."
og:image: https://openrouter.ai/dynamic-og?title=Provider%20Integration%20(Legacy%20Format)&description=OpenRouter%20Documentation
og:image:width: 1200
og:image:height: 630
...
nofollow: false
icon: cloud
sidebarTitle: For Providers (Legacy)
---

## For Providers (Legacy Format)

<Note>
  This page documents the legacy flat model document format, kept for existing integrations. For
  all new integrations, use the current format on the
  [provider integration guide](/guides/community/for-providers).
</Note>
  ...
  "data": [
    {
      ...
      "service_tier": "priority", // "flex", "priority", or "fast"; omit for the default tier
...
**Weekly schedules** add `utc_days` — an array of UTC weekday names (`"monday"`, `"tuesday"`, `"wednesday"`, `"thursday"`, `"friday"`, `"saturday"`, `"sunday"`). It scopes the entry to those days, evaluated at the request instant in UTC. Combined with `utc_start`/`utc_end` it scopes the window; conditions in one entry AND together. A weekday-only peak window (base pricing all weekend) looks like this:
    ...
    "overrides": [
      {
        ...
        "utc_start": 30, // peak window: 00:30–16:30 UTC, weekdays only
        "utc_end": 1630,
        "prompt": "0.00000056",
        "completion": "0.0000022"
      }
...
- All peak windows must carry the same `utc_days` set (or none). Day-scoped entries without a `utc_start`/`utc_end` window — i.e. a whole day priced differently from base — are not billable yet.
...
If you sell more than one grade of capacity for a model, list one entry per tier under the same `id` and mark each non-default entry with `service_tier`:

```json lines
[
  { "id": "your-org/your-model", "pricing": { "prompt": "0.000001", "completion": "0.000002" } },
  { "id": "your-org/your-model", "service_tier": "flex", "pricing": { "prompt": "0.0000005", "completion": "0.000001" } },
  { "id": "your-org/your-model", "service_tier": "priority", "pricing": { "prompt": "0.000002", "completion": "0.000004" } }
]
```

Accepted values are `flex`, `priority`, and `fast`. `fast` is an alias for `priority`, so list one or the other for a given `id`, not both. An omitted field means the default tier. Any other value fails validation and the entry is dropped.

Behavior:

- OpenRouter creates a separate endpoint for each tier entry, using that entry's own pricing, context length, limits, and capabilities.
- A tier entry is staged only once OpenRouter serves the default tier of the same `id`. List the default entry first, or alongside the tier entries. Tier endpoints then go through the standard staging, baseline test, and unhide flow.
- Tier endpoints appear to users as [service tier endpoints](/guides/features/service-tiers) and stay out of default routing.
- `service_tier` is OpenRouter's vocabulary, not your API's. If your API names a tier differently, still publish it as `flex` or `priority` here. The mapping to your wire format lives in OpenRouter's integration for your API.
- An entry cannot combine `is_free: true` with `service_tier`.

For OpenRouter to serve a tier endpoint, your inference API must accept `service_tier` on the request and report the tier that served the request in the response, and OpenRouter's integration for your API must forward it. Tier endpoints stay hidden until that integration is in place, so coordinate with us before publishing tier entries.
