...
| `service_tier` | Service tier this document describes. See [Service tiers with `service_tier`](#service-tiers-with-service_tier) |
...
If you serve the same model at more than one service tier with its own pricing or capacity, publish one document per tier and set the optional `service_tier` field on every non-default one:
...
- Accepted values are `flex`, `priority`, and `fast`. `fast` is an alias of `priority`: it is displayed as declared but routed and billed as the priority tier, so a feed must not list both for the same model id.
- An omitted field describes the default tier.
- Each tier becomes its own endpoint on OpenRouter, so a document with `service_tier` is never matched against, or treated as a duplicate of, the default-tier document for the same `id`. Pricing and capacity on a tier document apply only to that tier.
- `is_free` combines with `service_tier` per tier: a free variant of a tier document is a free endpoint of that tier.
