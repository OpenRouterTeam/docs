---
title: 'Jev Tutorial: Make Your First Decision Call on OpenRouter'
sidebarTitle: Jev Tutorial
description: Step-by-step Jev tutorial. Send a typed decision request to the TypeSafe model on OpenRouter with curl, TypeScript, or Python and read the probabilities it returns
seoTitle: 'Jev Tutorial | Your First Call to the TypeSafe Decision Model'
og:site_name: OpenRouter Documentation
og:title: 'Jev Tutorial - Make Your First Decision Call on OpenRouter'
og:description: 'A step-by-step Jev tutorial. Make your first Choice, Noul, or Score call to the TypeSafe System One decision model with curl, TypeScript, or Python, then read the probabilities.'
og:image: https://openrouter.ai/dynamic-og?title=Jev%20Tutorial&description=Make%20your%20first%20decision%20call%20on%20OpenRouter
og:image:width: 1200
og:image:height: 630
...
If you want a few concepts before diving into code, start with the [Jev documentation hub](/guides/community/jev). If you already know Jev and want the full request schema, go to the [Decisions API reference](/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request).
...
The response contains one typed answer per question, plus usage. This is an actual response to the request above, captured from the live API. Your probabilities will differ slightly from run to run, and the field shapes are documented in the [Decisions API reference](/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request):
...
If you're already using the TypeSafe JavaScript or Python SDK, you'll just need to point it at OpenRouter and keep your existing code. The [Jev SDK guide](/guides/community/typesafe-sdk) covers setting the base URL, authentication, and the expected response shape.
...
You can also call the Decisions API through the OpenRouter [TypeScript](/client-sdks/typescript/sdks/decisions/README), [Python](/client-sdks/python/sdks/decisions/README), or [Go](/client-sdks/go/sdks/decisions/README) SDK.
...
- [Jev documentation hub](/guides/community/jev) for concepts, every Jev page on OpenRouter, and links to the TypeSafe docs
- [Gate Agent Tool Calls with Jev](/cookbook/building-agents/gate-tool-calls-with-jev) to approve, block, or escalate agent tool calls
- [Cut LLM Cost with a Jev-Verified Cascade](/cookbook/evaluate-and-optimize/jev-verified-cascade) to verify cheap-model answers before escalating
- [Classify and Tag Text at Scale with Jev](/cookbook/evaluate-and-optimize/jev-classification) to run the Choice and Noul questions from this tutorial over a whole batch and pick thresholds from labeled data
- [Auto-Approve Coding Agent Permission Prompts with Jev](/cookbook/coding-agents/auto-approve-permission-prompts-with-jev) to put a Noul threshold in front of Claude Code, Codex, Cursor, or OpenCode permission prompts
