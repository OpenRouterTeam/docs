...
| `decisions` | Decision models such as [Jev](https://openrouter.ai/typesafe/jev-1.13) that return typed judgments instead of text. See [Jev Lab](https://openrouter.ai/labs/jev) and the [Jev gating recipe](/cookbook/building-agents/gate-tool-calls-with-jev) |
...
[Variant suffixes](/guides/routing/model-variants/overview) are also supported. A catalog variant such as `:free` returns that variant's own entry. A routing variant such as `:nitro` returns the base model's entry, because routing variants are not separate catalog entries:
