...
description: Call OpenRouter from workloads signed in with your own identity provider, without long-lived API keys
seoTitle: 'Workload Identity Federation | Keyless Authentication for OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Workload Identity Federation - Keyless Authentication for OpenRouter'
og:description: 'Exchange a JWT from your identity provider for a short-lived OpenRouter access token using RFC 8693 token exchange. No long-lived API keys in your workloads.'
og:image: https://openrouter.ai/dynamic-og?pathname=guides/overview/auth/workload-identity-federation&title=Workload%20Identity%20Federation&description=Keyless%20authentication%20for%20OpenRouter
og:image:width: 1200
og:image:height: 630
...
<Note>
  Workload identity federation is available on Business and Enterprise plans. Organizations on those plans find it under **Settings → Workload identity**. Organization admins can upgrade to the Business plan under **Account Type** in [Settings → Preferences](https://openrouter.ai/settings/preferences). For Enterprise, [contact our enterprise team](https://openrouter.ai/enterprise/form).
</Note>
...
```bash title="Inference with an access token"
