...
You can set and update all three filters programmatically using the [BYOK management endpoints](/api/api-reference/byok/list-byok-provider-credentials). The API uses these field names:
...
| API key filter | `allowed_api_key_hashes` | `string[] \| null` | Allowlist of OpenRouter API key hashes (the `hash` field from the [Keys API](/api/api-reference/api-keys/create-a-new-api-key)). `null` means no restriction. |
...
- Must be an array of strings (the SHA-256 `hash` values returned by the [Keys API](/api/api-reference/api-keys/create-a-new-api-key)).
