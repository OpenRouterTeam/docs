...
This means a BYOK key does not exempt a provider from your `data_collection` restrictions. For [Zero Data Retention](/guides/features/zdr) the same is true by default: if you enforce ZDR (via `provider.zdr`, account privacy settings, or a guardrail) and a provider's endpoint retains prompts, that endpoint stays ineligible even when you supply your own key. The exception is a key with a ZDR declaration in its **Provider agreement** section: declaring your account has ZDR with the provider stamps your BYOK copy with a ZDR policy so it survives your ZDR guardrail (video generation remains excluded).
...
To use a provider via BYOK, make sure it's permitted by your data policies: the provider's endpoint must satisfy any ZDR or `data_collection` restrictions you've enabled, either through the provider's default policy or a ZDR declaration on the key. See [Zero Data Retention](/guides/features/zdr) and [Provider Routing](/guides/routing/provider-selection).
