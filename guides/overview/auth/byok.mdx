...
On a regional domain, OpenRouter routes only to endpoints it has onboarded for that region (see [In-Region Routing](/guides/features/in-region-routing)). OpenRouter cannot see where your own OpenAI account processes requests, so you tell OpenRouter. The **Provider agreement** section on an OpenAI key card has a **Data region** setting, next to the ZDR setting, on the [OpenAI provider detail page](https://openrouter.ai/workspaces/default/byok/openai). It has three options, each labelled with the OpenRouter hostname it applies to:
