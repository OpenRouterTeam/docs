...
- `resolution` &mdash; normalized tier (`512`, `1K`, `2K`, `4K`). Concrete pixel dimensions are derived per-provider.
- `aspect_ratio` &mdash; normalized ratio. Pass `auto` to let the provider choose. Common values include `1:1`, `16:9`, `9:16`, `4:3`, `3:4`, `3:2`, `2:3`, `4:5`, `5:4`, and extended ratios like `1:2`, `2:1`, `1:4`, `4:1`, `1:8`, `8:1`, `9:21`, `21:9`. Providers clamp to their supported subset — check the model's `supported_parameters` for accepted values.
- `size` &mdash; convenience shorthand. Pass a tier (`"2K"`) or explicit pixels (`"2048x2048"`) and it gets normalized for the provider. Interchangeable with `resolution` + `aspect_ratio`; conflicting values are rejected.
...
- `output_format` &mdash; `png`, `jpeg`, or `webp`.
- `background` &mdash; `auto`, `transparent`, or `opaque`. `transparent` requires an alpha-capable format (png or webp).
- `output_compression` &mdash; 0&ndash;100 for webp/jpeg. Ignored for png.
...
**Completed** &mdash; emitted when the final image is ready. For raster PNG outputs, `media_type` is omitted:
```
...
| `n` | integer | No | Number of images to generate (1&ndash;10) |
...
| `output_compression` | integer | No | Compression level (0&ndash;100) for webp/jpeg |
