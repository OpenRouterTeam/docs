...
| `voice` | string | Provider-dependent | Voice identifier. Available voices vary by model — check each model's page on the [Models page](/guides/overview/models) for supported voices. Omit this parameter only when the selected provider documents a default voice; otherwise an explicit voice is required. |
...
Note: some providers for a voice-cloning model may not support voice cloning — check the `supports_voice_cloning` field on the [endpoints API](/api/api-reference/endpoints/list-all-endpoints-for-a-model).
