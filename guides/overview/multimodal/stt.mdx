...
To find the slug for each provider serving a model, call the [endpoints API](/api/api-reference/endpoints/list-all-endpoints-for-a-model). The `tag` field of each endpoint record is the key to use under `provider.options`:
...
```json title="Response (abridged)" lines
{
  "language": "en",
  "duration": 6.4,
