...
| `409` | `busy` | Another turn is already running **on this intern**. The lock is per intern, not per session, so opening a second session does not let you run two turns at once. That is deliberate: two sessions on one intern share the same workspace files, so serialising them is what stops a turn overwriting work another is mid-way through. |
