...
description: Stream a chat completion from one of your interns with an API key, and answer its questions and permission requests in a second request
seoTitle: 'Chatting with Interns | OpenRouter Documentation'
og:site_name: OpenRouter Documentation
og:title: 'Chatting with Interns'
og:description: 'Stream OpenAI-compatible chat completions from your intern, and answer its questions and permission requests through the same session.'
nofollow: false
icon: messages
---
...
| `408` | `timeout` | The request exceeded the route's own deadline before the handler answered. Distinct from `504`, which is the intern failing to answer within the turn budget. |
| `409` | `intern_not_ready` | The intern is not running, or was provisioned before chat was available. |
| `409` | `busy` | A turn is already running **in this session** and did not take the message within 15 seconds, for instance because it is waiting on a question of its own. The message was withdrawn, so it will not run later. Sessions on one intern run in parallel, so open another session to run a second turn at the same time; bear in mind that sessions on one intern share the same workspace files. |
...
`error.metadata.retryable` is a boolean beside `reason`, and it is the field to branch on when deciding whether to send the same request again. It is `true` only where the cause is transient by construction:

| `retryable` | Reasons |
| --- | --- |
| `true` | `busy`, `intern_not_ready`, `intern_unreachable`, `rate_limited`, `stream_severed`, `timeout` |
| `false` | `attachment_failed`, `bad_request`, `client_closed_request`, `interaction_not_pending`, `interaction_unknown`, `intern_rejected`, `not_found`, `payload_too_large`, `run_ended` |

`turn_failed` is the exception and carries no fixed value. One `turn.failed` event covers both an upstream overload worth retrying and an invalid provider key that will be refused identically forever, so the intern classifies its own failure and that answer rides on the response.

Where a wait is known, `Retry-After` carries it in seconds; where it is not, retry with your own backoff rather than immediately.

Responses from the shared authentication and membership checks (`401`, `403`, and the programme `404`) carry `error.code` and `error.message` without `metadata`, so treat a missing `metadata` as "not retryable" rather than as an error in itself.
