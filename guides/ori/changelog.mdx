---
title: Changelog
description: Curated release notes for the Ori CLI, with version history and changes across releases.
seoTitle: 'Ori Changelog | CLI Release Notes'
og:site_name: OpenRouter Documentation
og:title: 'Ori Changelog - CLI Release Notes'
og:description: 'Read Ori CLI release notes, from the first tagged release through the latest version.'
og:image: https://openrouter.ai/dynamic-og?title=Ori%20Changelog&description=Ori%20CLI%20release%20notes
og:image:width: 1200
og:image:height: 630
...
Curated release notes for the Ori CLI, newest version first. If you are new to Ori, start with [Ori Eval](/guides/ori/eval), then [Ori Harness](/guides/ori/harness), which also covers installing Ori and running your existing agent CLI. [Where Ori writes files](/guides/ori/files) explains what Ori puts in your project.
...
- **Quality gates.** lintcn integrates into CI with a no-isrecord rule. The sentrux fork pin fixes max_depth counting for type-only edges, and markdownlint disables its unfixable MD060 rule.
...
- **Pi message decoding and usage.** The pi-ACP adapter decodes the user's own message_end instead of logging it as malformed. It surfaces pi token usage and cost. pi turns are priced from OpenRouter's streamed usage.
...
- **web_fetch SSRF.** DNS resolves before the SSRF check, and the socket pins to the vetted IP to close the TOCTOU gap.
