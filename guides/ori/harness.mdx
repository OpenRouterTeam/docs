...
Configure [guardrails](/guides/features/guardrails), allowlists, and per-workspace permissions on your OpenRouter organization. Set [workspace budgets](/guides/features/workspaces/workspace-budgets) there too. OpenRouter enforces these controls on every request, whatever client makes it—including an agent running through Ori.

## One bill across agents

Ori traffic uses OpenRouter's normal billing and activity reporting. It appears alongside your other OpenRouter traffic in [Usage Accounting](/cookbook/administration/usage-accounting) and [Activity Export](/cookbook/administration/activity-export). There is no separate Ori bill.
