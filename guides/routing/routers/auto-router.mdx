...
og:description: "Automatically select the best AI model for your prompts using OpenRouter's Auto Router powered by task type rankings."
...
- **[`provider.max_price`](/guides/routing/provider-selection#max-price)** — a per-request price ceiling. The Auto Router resolves models before provider routing runs, so the selected models' endpoints are still filtered by `max_price`; if no endpoint satisfies the ceiling, the request fails instead of exceeding it.
