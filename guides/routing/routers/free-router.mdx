...
- **Rate limits**: Requests routed through `openrouter/free` count toward the same account-wide free-model rate limits as the [`:free` variant](/guides/routing/model-variants/free#rate-limits). The daily cap depends on whether you have ever purchased credits — see [Rate limits](/api_reference/limits#rate-limits) for the exact numbers and how they scale.
