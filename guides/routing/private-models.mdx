...
A private deployment may be eligible for in-region routing when OpenRouter is able to derive a region from its deployment URL. Deployments without a derived region are routed on the global `openrouter.ai` domain. See the [In-Region Routing guide](/guides/features/in-region-routing#byok-with-in-region-routing) for supported providers.
