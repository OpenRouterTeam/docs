...
You can append `:nitro` to any model slug as a shortcut to sort by throughput. In addition to the throughput sort, `:nitro` makes [priority service tier](/guides/features/service-tiers) endpoints eligible for the request, so it is a superset of setting `provider.sort` to `"throughput"` (which only sorts). `:nitro` is a [routing variant](/guides/routing/model-variants/overview): it is not a separate entry in the models API, and the model's metadata is the base model's. See the [Nitro variant docs](/guides/routing/model-variants/nitro) for details.
...
You can append `:floor` to any model slug as a shortcut to sort by price. In addition to the price sort, `:floor` makes [flex service tier](/guides/features/service-tiers) endpoints eligible for the request, so it is a superset of setting `provider.sort` to `"price"` (which only sorts). `:floor` is a [routing variant](/guides/routing/model-variants/overview): it is not a separate entry in the models API, and the model's metadata is the base model's. See the [Floor variant docs](/guides/routing/model-variants/floor) for details.
...
**Per-Model-Group and Account-Wide ZDR**
  ...
  ZDR can also be enforced per model group (Anthropic, OpenAI, Google, SpaceXAI, and all other models) in your [privacy settings](https://openrouter.ai/settings/privacy) or via [guardrails](/guides/features/guardrails). The per-request `zdr` parameter
