{/*
  ...
  Generated from monorepo source by scripts/generate-docs-constants.ts.
  Run `bun run generate:docs:constants` to regenerate.
  projects/docs must not import from other packages, so these constants are
  emitted here as a standalone snippet and refreshed on merge to main.
*/}
...
export const MISTRAL_OCR_US_USER_COST_PER_1K_PAGE_DOLLARS = 2.2;
