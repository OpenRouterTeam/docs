...
- [Gate Agent Tool Calls with Jev](/cookbook/building-agents/gate-tool-calls-with-jev) replaces the hand-written threshold in `onToolCalled` with a decision model that returns `approve`, `block`, or `review` for each call, so only ambiguous calls reach the pause path built here.
