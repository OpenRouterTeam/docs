...
<Note>
If `speed: "fast"` is sent for a model that does not support fast mode, it still requests the [priority service tier](/guides/features/service-tiers#fast-mode) where a provider offers one (at priority pricing); otherwise the request proceeds at standard speed with standard pricing.
</Note>
