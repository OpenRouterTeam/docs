...
3. Send `service_tier: "fast"` — the [`fast` service-tier alias](/guides/features/service-tiers#the-fast-alias) behaves like `speed: "fast"` on these models, and additionally requests the priority service tier (including its provider sorting and pricing) where available.
