...
description: Use the Codex desktop app (ChatGPT for macOS and Windows) with OpenRouter
seoTitle: 'Codex Desktop App Integration | OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Codex Desktop App Integration - OpenRouter'
og:description: 'Learn how to configure the Codex desktop app (ChatGPT for macOS and Windows) to route requests through OpenRouter.'
og:image: https://openrouter.ai/dynamic-og?title=Codex%20Desktop%20App&description=Use%20the%20Codex%20desktop%20app%20with%20OpenRouter
og:image:width: 1200
og:image:height: 630
...
<Note>
With `env_key` authentication Codex does not fetch the OpenRouter model catalog, so non-OpenAI models may show an "Unknown model" fallback-metadata warning. The command-based `auth` block on the [Codex CLI page](/cookbook/coding-agents/codex-cli#step-3-configure-codex-for-openrouter) triggers the catalog refresh and works in the desktop app as long as `OPENROUTER_API_KEY` is visible to it as described above.
</Note>
...
- [Codex CLI with OpenRouter](/cookbook/coding-agents/codex-cli)
