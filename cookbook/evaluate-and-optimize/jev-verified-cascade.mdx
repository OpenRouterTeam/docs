---
title: Cut LLM Cost with a Jev-Verified Cascade
description: Verify cheap-model answers against retrieved context, then escalate only when the answer isn't supported by that context.
seoTitle: 'Cut LLM Cost with a Jev-Verified Cascade | OpenRouter Cookbook'
og:site_name: OpenRouter Documentation
og:title: 'Cut LLM Cost with a Jev-Verified Cascade'
og:description: 'Draft with a cheap model, verify with Jev, and escalate only when the answer fails the check.'
og:image: https://openrouter.ai/dynamic-og?title=Jev-Verified%20Cascade&description=Draft%20cheaply%2C%20verify%2C%20and%20escalate%20on%20failure
og:image:width: 1200
og:image:height: 630
...
- Keep the [Decisions API reference](/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request) open for the verification endpoint.
</Tip>
...
<Info>
**Confidence.** Confidence is Jev's probability that the chosen label is correct. It ranges from 0 to 1 and appears only with `choice` and `score` answers. `probabilities` has one value per label. The [Decisions schema](/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request) marks both optional, so the code above defaults a missing confidence to `0`, which fails acceptance in step 3, and a missing probabilities object to `{}`.
</Info>
...
- **Endpoint.** Requests from the `decisions` client reach `https://openrouter.ai/api/alpha/decisions`. A client without `serverURL` gets a 404 for this call.
...
- [Gate Agent Tool Calls with Jev](/cookbook/building-agents/gate-tool-calls-with-jev) to apply the same approve, block, or review pattern to agent tool calls
- [Classify and Tag Text at Scale with Jev](/cookbook/evaluate-and-optimize/jev-classification) to run `choice` and `noul` questions over a batch and pick thresholds from labeled data
- [Jev SDK for TypeScript and Python](/guides/community/typesafe-sdk) to call Jev through the TypeSafe SDK
- [Jev documentation hub](/guides/community/jev) for the [Jev tutorial](/guides/community/jev-tutorial), the other cookbooks, and the TypeSafe docs on writing questions
...
- [RAG with Embeddings & Rerank](/cookbook/evaluate-and-optimize/rag)
- [Add Human-in-the-Loop Controls to an Agent SDK Agent](/cookbook/building-agents/hitl-tools)
