---
title: Classify and Tag Text at Scale with Jev
description: Run Jev text classification in batches with Choice for exclusive categories and one Noul per tag, pick thresholds from a labeled sample, and compute cost per 1,000 items from real usage.
seoTitle: 'Classify and Tag Text at Scale with Jev | OpenRouter Cookbook'
og:site_name: OpenRouter Documentation
og:title: 'Classify and Tag Text at Scale with Jev'
og:description: 'Batch text classification with Jev on OpenRouter: exclusive categories, multi-label tags, threshold tuning from a labeled sample, and cost per 1,000 items.'
og:image: https://openrouter.ai/dynamic-og?title=Jev%20Classification&description=Classify%20and%20tag%20text%20at%20scale
og:image:width: 1200
og:image:height: 630
...
- You've followed the [Jev tutorial](/guides/community/jev-tutorial) or read the [Decisions API reference](/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request) once, so the `state` and `questions` request shape is familiar.
- Keep the [rate limits page](/api_reference/limits) open for the status codes that step 3 retries.
</Tip>
...
<Warning>
Every call to `classifyPost` is a paid request. At the time of writing, Jev on OpenRouter costs \$0.042 per million input tokens with no output-token charge, and the current price is on the [model page](https://openrouter.ai/typesafe/jev-1.13). One kind of `402` is transient: when your running requests fill the [in-flight spending budget](/api_reference/limits#in-flight-spending-budget), the body includes `limit_source: "openrouter_in_flight_budget"` and the code retries after `Retry-After`. Any other `402` means your credits or key limit ran out, so the item fails and `runBatch` stops the batch.
</Warning>
...
- [Classify Reddit and YouTube Comments with Jev and ScrapeCreators](/cookbook/evaluate-and-optimize/classify-reddit-comments-with-jev) to point the same `choice` and `noul` questions at scraped comments and re-tune thresholds on stored judgments
- [Gate Tool Calls with Jev](/cookbook/building-agents/gate-tool-calls-with-jev) to apply the same `noul` thresholds to agent actions
- [Cut LLM Cost with a Jev-Verified Cascade](/cookbook/evaluate-and-optimize/jev-verified-cascade) to verify generated answers with a `choice` question
- [Jev documentation hub](/guides/community/jev) for every Jev page on OpenRouter and the TypeSafe docs on writing questions and choosing thresholds
- [TypeSafe SDK guide](/guides/community/typesafe-sdk) to call Jev through the TypeSafe JS or Python SDK instead of raw `fetch`
- [Decisions API reference](/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request)
