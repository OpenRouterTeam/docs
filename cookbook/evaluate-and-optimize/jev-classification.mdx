...
- You've read the [TypeSafe SDK guide](/guides/community/typesafe-sdk) or the [Decisions API reference](/api/api-reference/alphadecisions/submit-a-decisions-questions-and-answers-request) once, so the `state` and `questions` request shape is familiar.
