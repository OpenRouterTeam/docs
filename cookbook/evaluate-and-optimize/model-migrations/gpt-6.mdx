---
title: GPT-6 Migration Guide
description: Adopt async tool calling and mid-conversation reasoning effort changes with configuration_update for the GPT-6 model family
seoTitle: "GPT-6 Migration Guide | OpenRouter"
og:site_name: OpenRouter Documentation
og:title: "GPT-6 Migration Guide"
og:description: >-
  Learn how to use async tool calling on the Responses API, change
  reasoning effort mid-conversation with configuration_update, and handle
  safety policy blocks on GPT-6 Astra, Sol, and Luna across the Chat Completions,
  Responses, and Anthropic Messages APIs on OpenRouter.
og:image: https://openrouter.ai/dynamic-og?title=GPT-6%20Migration%20Guide&description=Async%20tool%20calling%20and%20mid-conversation%20reasoning%20effort%20for%20Astra%2C%20Sol%2C%20and%20Luna
og:image:width: 1200
og:image:height: 630
...
The tiers share one API. Sol and Luna accept `none` as a reasoning effort, Astra does not. Pro reasoning is selected with `reasoning.mode: "pro"` on a standard slug or by using the `-pro` slug, the same as the [GPT-5.6 family](/cookbook/evaluate-and-optimize/model-migrations/gpt-5-6#pro-mode-with-reasoningmode).
...
- Mid-conversation effort updates are not accepted on the [Batch API](/batch-quickstart) and are rejected per line.
...
A refusal that the model produces as its own completed output is not an error. The Responses API returns `status: "completed"` with a `refusal` content part, and Chat Completions returns `message.refusal` with `finish_reason: "content_filter"`. See [Errors and Debugging](/api/api-reference/errors-and-debugging#content-policy) for the full contract.
...
- [GPT-5.6 Migration Guide](/cookbook/evaluate-and-optimize/model-migrations/gpt-5-6) for `reasoning.mode`, `reasoning.context`, and explicit prompt caching, which carry over to GPT-6
...
- [Tool Calling on the Responses API](/api/api-reference/responses/tool-calling) on OpenRouter
- [Errors and Debugging](/api/api-reference/errors-and-debugging#content-policy), the OpenRouter error contract for policy blocks and refusals
- [Changing Effort Mid-Conversation](/guides/best-practices/reasoning-tokens#mid-conversation-effort) in the OpenRouter reasoning guide
