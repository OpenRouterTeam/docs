...
description: Migrate to Claude Fable 5.1 — ephemeral system messages, per-turn effort, forced tool use rejected, mid-thinking display updates, and prefix-locked thinking
seoTitle: 'Claude Fable 5.1 Migration Guide | OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Claude Fable 5.1 Migration Guide'
og:description: 'Learn what changed in Claude Fable 5.1 — ephemeral mid-conversation system messages, per-turn effort changes, forced tool use rejection, mid-thinking display updates, and prefix-locked thinking.'
og:image: https://openrouter.ai/dynamic-og?title=Claude%20Fable%205.1%20Migration%20Guide&description=Ephemeral%20system%20messages%2C%20per-turn%20effort%2C%20forced%20tool%20use%20rejection%2C%20and%20display%20updates
og:image:width: 1200
og:image:height: 630
...
The same change is available through the Chat Completions API (`configuration_update` on a content-less system message) and the Responses API (a `configuration_update` input item). OpenRouter translates each form into the `output_config` message Fable expects. See [Changing Effort Mid-Conversation](/docs/guides/best-practices/reasoning-tokens#mid-conversation-effort).
