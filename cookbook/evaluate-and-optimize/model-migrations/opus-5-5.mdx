...
description: Migrate from Claude Opus 5 to Claude Opus 5.5. Adaptive-only thinking with effort as the main control, forced tool use retired, mid-thinking display updates, preserved thinking, and 20% lower pricing
seoTitle: 'Claude Opus 5.5 Migration Guide | OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Claude Opus 5.5 Migration Guide'
og:description: 'Learn what changed from Claude Opus 5 to Claude Opus 5.5. Adaptive-only thinking, effort as the main control, forced tool use retired, mid-thinking display updates, preserved thinking, and lower pricing.'
og:image: https://openrouter.ai/dynamic-og?title=Claude%20Opus%205.5%20Migration%20Guide&description=Adaptive-only%20thinking%2C%20forced%20tool%20use%20retired%2C%20and%20lower%20pricing
og:image:width: 1200
og:image:height: 630
...
Claude Opus 5.5 ships as [`anthropic/claude-opus-5.5`](https://openrouter.ai/anthropic/claude-opus-5.5). Most Opus 5 prompts work unchanged. The API changes are the same set that arrived with [Fable 5.1](/cookbook/evaluate-and-optimize/model-migrations/fable-5-1), plus one that is new for the Opus line.
...
- **Per-turn reminders or mid-session system prompt changes**: append a mid-conversation system message instead of editing the system prompt, and for a one-turn reminder mark it ephemeral with `clear_at: "next_user_message"`. Anthropic states this applies to Opus 5.5 as it does to Fable 5.1, see [Ephemeral Mid-Conversation System Messages](/cookbook/evaluate-and-optimize/model-migrations/fable-5-1#ephemeral-mid-conversation-system-messages-beta).
- **Adding or removing tools**: declare the full set at session start and send a [mid-conversation tool-change block](/cookbook/evaluate-and-optimize/model-migrations/opus-5#mid-conversation-tool-changes-beta) instead of changing `tools`.
...
Changing effort between requests on a conversation invalidates the prompt cache. Opus 5.5 supports the per-turn form introduced with Fable 5.1: a system-role message with `output_config.effort` that applies to the next assistant turn only, without a cache bust. Details and request shape are in the [Fable 5.1 guide](/cookbook/evaluate-and-optimize/model-migrations/fable-5-1#per-turn-effort-changes-beta).
...
- [Claude Fable 5.1 Migration Guide](/cookbook/evaluate-and-optimize/model-migrations/fable-5-1), `clear_at` ephemeral system messages and per-turn effort in full
- [Claude Opus 5 Migration Guide](/cookbook/evaluate-and-optimize/model-migrations/opus-5), mid-conversation tool changes
