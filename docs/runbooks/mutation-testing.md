...
| `--max-minutes <n>` | `10` | Wall-clock cap for the whole run, `0` to lift it. When it is spent, the engine is killed, the mutated files are restored from its backups, and the run exits non-zero listing the packages it did not finish. Narrow the file list, add `--incremental`, or raise the cap to measure more. CI passes `0` because its step timeout is the ceiling there. |
