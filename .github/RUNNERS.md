...
Composite setup actions cannot read the `vars` context, so they detect the fleet from `RUNNER_NAME` (`warp-*` is WarpBuild) through the shared `.github/scripts/detect-fleet.sh` and export `IS_WARPBUILD` to the job environment. `setup-environment-cached` mounts sticky disks only on Blacksmith runners and caches the bun store and turbo cache through WarpCache on WarpBuild runners, so a workflow that calls it and passes `sticky-disks-enabled: ${{ vars.USE_STICKY_DISKS }}` needs only its `runs-on` expression converted. `use-sticky-disks: "false"` disables the WarpCache steps as well, so artifact-producing workflows stay cold on every fleet. A local action resolves against the checked-out tree, not the workflow revision, so workflows that check out another ref (`openapi-pr-comment.yaml` checks out the PR head branch, `time-cloudflare-worker-upload.yaml` checks out `client_payload.ref`) call the compatibility alias `setup-environment-blacksmith`, which exists on trees from before the rename and forwards to `setup-environment-cached` on trees after it.
