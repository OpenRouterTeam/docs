...
metaDescription: "How to use Jev on a real problem: split code from judgment, shape the state, write literal questions, call the Jev API from TypeScript, and tune thresholds."
teaser: "A method for putting Jev to work on a new problem, worked end to end on marketplace listing moderation: what stays in code, what Jev sees, how to phrase the questions, and how to turn its probabilities into publish, hold, or reject."
headerImage:
  url: "/images/how-to-use-jev.png"
  width: 1376
  height: 768
faq:
  - question: "How do I use Jev with OpenRouter?"
    answer: "Create an OpenRouter API key, install @openrouter/sdk, and call openrouter.alpha.decisions.create() with model typesafe/jev-1.13, a state object, and one or more typed questions (choice, noul, or score). Jev returns a typed answer with probabilities for each question, and your code turns those numbers into actions. The same request works over plain HTTP at POST https://openrouter.ai/api/alpha/decisions."
  - question: "What is the Jev API endpoint and model ID?"
    answer: "Jev is served through OpenRouter's Decisions API at POST https://openrouter.ai/api/alpha/decisions. The model ID is typesafe/jev-1.13, and the alias ~typesafe/jev-latest tracks the newest release. The request body carries the model, state, and questions."
  - question: "Do I need a TypeSafe account to get access to Jev?"
    answer: "No. Anyone with an OpenRouter API key can use Jev, billed to your OpenRouter account. There's no waitlist, and you don't need a separate TypeSafe signup."
  - question: "What confidence threshold should I use with Jev?"
    answer: "Pick it from your own labeled data and the cost of a wrong action, since Jev's confidence measures how concentrated the probability distribution is and says nothing about accuracy on your domain. Judge a labeled sample once, save the answers, and replay candidate thresholds over them. In our 24-listing sample, every real violation scored 0.99 or higher, and 0.8 routed the two borderline cases to a reviewer."
  - question: "What should I use Jev for, and what should stay in code?"
    answer: "Jev is good at semantic judgments on a bounded set: which category something belongs to, whether a proposition is true, and where it falls on a scale you define. Keep arithmetic, counting, date comparisons, lookups, and execution in your code, and feed the results to Jev as state. Jev can't generate text, so tasks that involve writing still need a generative model."
howTo:
  name: "Moderate marketplace listings with Jev on OpenRouter"
  totalTime: "PT45M"
  steps:
    - name: "Split the checks between code and Jev"
      text: "List every check your moderation flow makes. Anything with a deterministic answer (price bounds, photo count, field lengths, price relative to category) stays in plain code. Anything that requires reading and interpreting the listing goes to Jev."
    - name: "Shape the state Jev sees"
      text: "Build a state object with only the fields the questions need, using descriptive names such as listing.declared_condition, plus facts your code computed such as price_signal and category_definition."
    - name: "Write the questions and criteria"
      text: "Define a choice question for prohibited-item type with an explicit none option, noul questions for category fit, title and description contradiction, and off-platform contact, and a score question over five concrete condition levels."
    - name: "Call the Decisions API from TypeScript"
      text: "Run bun add --exact @openrouter/sdk@1.3.17, set OPENROUTER_API_KEY, and call openrouter.alpha.decisions.create() with model typesafe/jev-1.13. Narrow each answer on its type field and read usage.cost."
    - name: "Turn probabilities into publish, hold, or reject"
      text: "Write a pure decide() function that applies thresholds to the saved judgment and returns an action with reasons. Run hard rules first, then Jev, then decide(), and route any failed judgment to hold."
    - name: "Tune thresholds on a labeled sample"
      text: "Judge a hand-labeled set once, then replay candidate thresholds over the saved answers and print auto-rejects, wrong rejects, and items sent to review for each threshold."
    - name: "Run the full flow"
      text: "Run a few sample listings through moderate() and compare the printed actions, reasons, and cost with the transcript in the article."
---
