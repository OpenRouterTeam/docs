...
To pin the batch to a provider, add a top-level `provider` object with an `only` array of [provider slugs](/guides/routing/provider-selection), placed before `requests`:
...
```shell title="Creation-time range" lines
