...
seoTitle: 'Tool Approval & State Persistence | OpenRouter Agent SDK'
...
- **[Gate Agent Tool Calls with Jev](/cookbook/building-agents/gate-tool-calls-with-jev)** - Decide `approve`, `block`, or `review` per call with a decision model instead of a static `requireApproval` flag
