...
Production splits by chunk rate and client mode with the `"Transaction attempt"` log: `@extra.user_streamed` is the client flag (`false` for 74% of requests and ~63% of all cfw-api CPU as of 2026-09-03), `@extra.streamed` is the upstream flag (always true on the SSE adapters), and `@breadcrumbs.chunks_received_from_last_stream` counts upstream SSE events. Bucket `@wallTime` at a fixed chunk-count band to see the pacing gradient. The Infisical `/services/cfw-api` Datadog keys are logs-write-scoped and 403 on every read endpoint, including dashboards; a personal access token (`ddpa…`, sent as `Authorization: Bearer`) reads dashboards, logs search, and logs aggregate.
