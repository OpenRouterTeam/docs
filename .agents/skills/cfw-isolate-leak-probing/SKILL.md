...
5. **Account failures honestly, and read the counts before the slope.** An arm that reports `failures=0` because failures were miscounted as successes will still produce a confident-looking slope out of requests that never reached the worker. Check `failures`, the status map, and the per-request byte/chunk averages at every checkpoint — a shape whose bytes drift is no longer one shape. On an abort arm specifically, note that the intended `abort()` does *not* surface as an exception (nothing is awaited past it), so any exception on that path is a real failure. Three outcomes, three counts: a `fetch()` that rejects before headers is indeterminate, not proof the request was never served — the worker may have allocated and then died — so the arm publishes no verdict and reachability is settled from an independent signal (the worker log line, a request ID echoed by the route, or the isolate's own kill record); a body that tears after headers kept its status and its allocations, so it is measured but is not a success (a 2xx header with a torn body counted as `ok` is how a memory-triggered kill hides); a status the worker chose to return is a served request whatever the code.
...
**A local large-body arm cannot resolve `BaseAdapter.fetch` body retention.** `sendToFSLog('adapters/base-fetch-request', { request: upstreamRequest })` re-stringifies the whole body under `waitUntil`, and `isFSLoggingEnabled()` is on outside production and test (`shouldEnableFsLogging` in `packages/clients/fs-logs/fs-logging-gate.ts`; the HIPAA mirror is on only under `OR_ENV=development`) — so local dev carries a second full copy on exactly the path being measured. Confirm whether disabling fs-logging moves the baseline before reading such an arm at all.
...
`Router.#releaseMultiModalRequestInputs()` (`packages/router/index.ts` (#releaseMultiModalRequestInputs)) is the mechanism, and by enumerating what it clears it documents that a multimodal payload has **five simultaneous holders**: `_rawRequest`, `_normalizedRequest`, `_currentAdapterRequest`, `#cachedEstimator.allImages`, and `_currentAdapter.estRequestTokens.{images,input}` — plus adapter-internal fields via `releaseRetainedRequestInputPayloads()`. Its own comments warn that some of these **share the same part objects** while the estimator ones are genuine Zod-parsed copies, so a bytes model must distinguish aliases from copies or it will overstate by several multiples. That comment is the authority on which is which.

**The retention is intentional and the trigger is the interesting part.** There is exactly one call site, passed as `onFirstContentWrite` to `flushAdapterResponse`, and it fires from `StreamBreaker` only on `#isContentChunk(chunk) && !didWriteTokens` (`packages/network/stream-breaker.ts`). The rationale is in the code: "Once a content chunk is written downstream the pipe is committed: no caller can retry with another provider. Notify so callers can release resources that were only retained to support fallback."
...
- *Release at 2xx instead of first content* — a **correctness bug**. Three producers land in the 2xx-to-first-content window and each re-serializes the payload to a *different* provider: pre-output refusal fallback, empty-response fallback, and any pre-first-token error becoming an invisible retry (all inside `packages/router/index.ts`, the refusal and empty-response paths in `packages/router/adapters/base/index.ts`). It would send `[OR_REDACTED]` images to the fallback provider.
...
So `flushAdapterResponse` (after the response is fully serialized but before first content write) is the earliest sound placement and **the residual exposure is inherent to supporting fallback**. Two sub-hypotheses died here too, both worth not re-chasing: adapters do **not** accumulate across a fallback chain (only one live adapter at a time — `_currentAdapter`, `_currentAdapterRequest` and `#cachedEstimator` are the sole slots and are nulled between attempts, so N-attempt retention is O(1) in payload bytes), and the opt-outs do **not** make the mechanism moot (classification samples at `DEFAULT_CLASSIFICATION_SAMPLE_RATE = 0.0025`, and `retainMultiModalRequestInputs()` has exactly two call sites — `shadow_models` requests and Mission Control/eval — so >99.7% of multimodal inference traffic is eligible).

**One live bug came out of this, on a different axis.** Because the release **mutates parts in place** and `find-matching-server-tools.ts` (the tool-match loop) stashes `_callerMessages = rawRequest.messages` by *reference*, the fusion and advisor tools can read `[OR_REDACTED]` where they intend to recover the user's attachments (`fusion-tool.ts` (the JSON.parse call), `advisor-tool.ts` (the two argument-parse sites)) — defeating the stated purpose in the docblock at `fusion-tool.ts` (the argument-normalization block). It is order-dependent on whether a content chunk preceded the tool call, so it presents as intermittently missing attachments rather than a hard failure. **In-place release plus an aliased stash is the general hazard**: any consumer that captured a reference before the release sees redacted data afterwards.

### The modality workers are swept and clean; the residue is unguarded schema edges

Two independent sweeps of `packages/embeddings`, `rerank`, `image-generation`, `video-generation`, `stt` and `packages/batch` / `services/batch-api` found **no unbounded same-dimension nested scan** — every multiplier is schema-bounded or genuinely linear. Notable: `services/batch-api/src/submit/` was written against this hazard class already (`BATCH_MODERATION_CHUNK_SIZE = 32` bounds the flush window, `run-batch-preflights.ts` (the Map-building loop) deliberately builds a `Map` rather than calling `.find()`, custom_id uniqueness is a `Set`). Two shapes that *read* as rebuilds and are not: `mergeAdapterSKUItems` (`services/cfw-video-api/.../video-generation-job.ts` (the mergeAdapterSKUItems call)) is passed `adapter.skuItems.slice(countBeforePoll)` — only that poll's delta, so total work is O(items) not O(polls × items), and that slice is load-bearing; and `resolveFrameUrl`'s `.find()` runs exactly twice per request across all six adapter sites, never inside a frame loop.

What the sweep did surface is not performance but **caps that are missing at the edge**:

- **An "uncapped" field may be uncapped by design — check for an offload path before proposing a cap, then check that path's *conditions*.** `STTInputAudioSchema.data` is a bare `z.string()` with no `.max()` (`packages/stt-interfaces/schemas/request/index.ts` (the request schema root)), which reads as a missing bound. It is not: large base64 audio is meant to be offloaded into a Durable Object rather than buffered (`packages/stt/app.ts` (the route registration block)), so a `.max()` would reject requests the architecture exists to handle. The real gap is one branch of the offload *condition*. `setupStreamingRequest` (`packages/supersize-streaming/setup-streaming-request-if-needed.ts` (the supersize hook)) offloads on an `x-offload-large-fields` header, on `content-length` over threshold, or on a **missing** `content-length` — that last only when `missingContentLengthBehavior !== 'buffer'`. The type defaults to `'stream'` (`:37`, `:54`) and STT is the repo's only opt-out to `'buffer'` (`services/cfw-stt-api/src/utils/setup-streaming-request-if-needed.ts` (the supersize hook)), so a chunked JSON upload with no `content-length` buffers an unbounded payload there while every other service streams it. **And then check the provenance before calling it a gap, because this one is deliberate.** `'buffer'` traces to [#20451](https://github.com/OpenRouterTeam/openrouter-web/pull/20451) — commit subject `fix(stt): require explicit offload gate without content length` — where a reviewer caught that streaming on a missing header would 400 any client omitting it. The reason it still holds: **STT offload is not transparent**, unlike chat or embeddings. Exactly one of 11 STT adapters sets `supportsOffloadedAudio = true` (`packages/stt/adapters/openai/index.ts` (the adapter entry), Groq inheriting), and once the parser substitutes a placeholder, `packages/stt/routes/transcribe.ts` (the stream setup) hard-400s every adapter that has not opted in. So switching to `'stream'` converts a chunked upload from "buffers unbounded but succeeds" into an **unconditional 400 for nine providers**. The DO path itself handles a chunked body fine (`process-stream-json.ts` (the frame-parse loop) forwards a `ReadableStream`), so the blocker is the adapter gate one layer up; the real fix is hoisting offload dispatch into `BaseSTTAdapter`.
...
- **Span attribute processing ahead of the sampling decision (Aug 2026).** The static reading is correct — `processAttributes` runs in the `SpanImpl` constructor (`packages/cloudflare/instrumentation/tracer.ts` (the span-attribute processing)) before `headSample()` at `:273`, unconditionally, and prod publishes at `HEAD_SAMPLING_RATE = 0.005`, so ~99.5% of spans sanitize attributes that are never exported. It is still **not worth fixing**: a direct microbenchmark puts it at 0.050 µs for a 5-key span and 0.29 µs for a 22-key span, so even at 120 spans/request the total is under 0.01 ms/req — roughly 50x below the keep bar. The hypothesis only looked large because it was sized from a sampling profiler's wall-deltas (see the wall-time-is-not-CPU trap in Environment traps above). Also worth recording: `forceSample()` (`context.ts` (the context-assembly block)) retroactively flips every span in `activeSpans` to sampled, so any "discard attributes when unsampled" or "return a `NonRecordingSpan`" design is *unsafe* regardless of the payoff; lazy sanitization in `#serialize()` is the only correct shape, and it optimizes ~0.01 ms/req.
...
**Case 1 — a near-miss that would have shipped a silent regression.** The claim was "the prompt classifier discards all image parts, so the enqueue can strip them." Evidence: `getTextFromMessage` (`packages/classifier/build-classifier-text.ts` (the classifier-text assembly)) filters to `part.type === 'text'` and drops the rest. True, and irrelevant — `buildClassifierInputFromMessages` returns `{ text, imageParts }`, and a *separate* extractor `getImagePartsFromMessage` (`:83`) deliberately pulls base64 data URLs out. Those feed a dedicated vision sub-classifier (`Gpt5NanoTaskTypeClassifier`, selected at `classifiers/task-type-classifier.ts` (the buildClassifiers confidence-map branch)) and get hashed into the tag cache key. A blanket text-only strip would have degraded every multimodal classification to text-only, invisibly to any test that only checks text.
...
**One residual that cannot be measured locally.** `classifyPrompt()` returns early on `!isProduction()`, so the classification consumer never runs on a local worker. In production it does, and it makes ~3 full payload copies — `JSON.stringify(opts)` over intact `messages` (`packages/queues/tasks/classify.ts` (the task handler)), then base64 (+1.33x) at `packages/queues/client.ts` (the enqueue entry), then an outer stringify — for an estimated ~3.5-4x payload post-response at roughly 0.25% of requests (`DEFAULT_CLASSIFICATION_SAMPLE_RATE = 0.0025`). That is a **fat tail rather than a shifted mean**, and it is the only surviving payload-scaling post-response candidate. Note `getText` already flattens image parts to `<image>`, so the `messages` field may be redundant with `prompt`.
...
## Shape sweep closes the Responses/tool/stream question (2026-09-03)

Devin's 7d prod pull (kill log `isolate_requests_served` p50 2,722 vs baseline 1,168, heavily overlapping; payload p50 5x apart) confirmed the residual `exceededMemory` killer is **peak, not retention**, and Responses API kills ran ~1.6x their traffic share — a shape the earlier sweeps never probed. A shape-parameterized `peak-probe-shapes.mjs` (this directory; `--shape mm|responses-text|responses-image|chat-tools|chat-stream`, `--endpoint`, everything else verbatim from `peak-probe.mjs`) swept all five shapes at C=1,2, 2 MB payloads:

| shape | per-in-flight C=1 → C=2 | ratio |
| --- | --- | --- |
| chat mm | 15.11 → 18.33 MB | 1.21x |
| Responses image | 18.59 → 15.25 MB | 0.82x |
| Responses text | 18.20 → 16.50 MB | 0.91x |
| chat + tools | 14.37 → 14.19 MB | 0.99x |
| chat streaming | 14.83 → 13.10 MB | 0.88x |

Caveat on the Responses text row: that sweep ran before the script honored `--payload-kb` for `responses-text`, so its body was the short `hello` input, not a 2 MB one. Its flatness is still a valid per-in-flight result, just not a payload-matched comparison against the other rows — re-run it if you need the matched number.

All flat: **no shape amplifies per-request cost under concurrency.** The Responses ~1.6x kill over-representation is not a per-request defect; the probe's 0 KB / 2 MB / 4 MB / 8 MB arms even showed chat-mm scaling *sub*linearly past 2 MB (18.2 → 30.2 → 32.3 MB), i.e. the naive per-copy model overestimates big payloads (V8 string slicing / rope sharing).

**New finding from the zero-payload control (the historical sweep never ran one at C>1):** the *fixed* per-request peak cost is ~13-18 MB regardless of shape, and it **stacks linearly with concurrency** (0 KB sweep C=1,2,4: 17.9 → 12.4 → 13.1 MB per in-flight). At ~66-70 MB idle baseline that is ~4 concurrent requests before the 128 MB wall even with *empty* bodies. Prod victims dying at 21% under 500 served and `est_prompt_tokens` p50 9.6k are consistent with this fixed cost stacking, not only with multimodal payloads.

## Fixed-cost attribution + overflow design (2026-09-03)

`heap-attribute.mjs` (this directory) answers where that fixed cost goes: a CDP sampling-heap window opened tightly around N requests, aggregated by module and by subtree. `request-weight.ts` is the admission-gate estimator the findings calibrate.

**The fixed cost is endpoint-catalog hydration, not request handling.** Sampling one 0 KB chat-completions request (window ≈ 210 ms, ~67 MB sampled):

| subtree | allocated |
| --- | --- |
| `_endpointsCachePgFetcher` (catalog hydration) | ~33-35 MB |
| — `initEndpointFromDb` under it (Endpoint construction) | ~33 MB |
| — `getPublicPricingFromStrategy` + `applyDiscountToPublicPricing` | ~11-46 MB |
| pg wire client (`_listen`/`parse`/`handleDataRow`/`parseRow`) | ~25 MB |
| `_modelsCachePgFetcher` (models catalog) | ~4-16 MB |
| Zod `parseSchema`/`safeParse` over hydrated rows | ~3-15 MB |

Totals scale ~linearly with C (C=4: ~264 MB sampled, same proportions). Per-in-flight peak (13-18 MB) is far below the sampled total because these are transient allocations — but they drive peak through GC pressure.
...
Platform research (2026-09-03; CF service-bindings docs, workerd PR #6382 `abortIsolate`):

- **No API selects or hands off to a specific isolate.** Placement is the runtime's decision. Soft eviction exists but is platform-initiated, not callable. `abortIsolate()` kills the current isolate — the opposite of rescuing in-flight work. Cache-key control shapes caching, not placement.
- **The supported "handoff" is a service-binding forward**: worker A calls `env.OVERFLOW.fetch(request)` when overloaded. The twin worker's isolates are placement-fresh (unbound workers are not co-located), so the same weight that would kill this isolate fits there. This is request-level shedding, not isolate steering — the origin still holds the body bytes during the forward.

**Design: isolate-internal admission gate + binding overflow (depth 1).**

1. Track `inflightWeightMbSum` in the isolate-scope counters `trackIsolateRequest` already maintains (+/- on request end), fed by `estimateRequestWeight(headers)` — content-length only, no body read, `14 MB + 7x` calibration. It returns `sized` / `unsized` / `malformed` rather than guessing: a chunked body has no readable weight, so `unsized` goes straight to the overflow/streaming path instead of being admitted against a fabricated number, and a content-length outside HTTP's `1*DIGIT` syntax is rejected, not repaired.
2. Admit only if `shouldAdmit({ inflightWeightMbSum, baselineMb, wallMb, candidateWeightMb })` passes (10% safety fraction).
3. Overflow = `env.SVC_CFW_API_OVERFLOW.fetch(request)` to a twin `cfw-api-overflow` worker (same script, separate name, env flag forbidding re-forward). Same stateless inference path, so no double-execution concern; twin sheds with 429 instead of forwarding again.
4. Streamed/large bodies are re-read from the origin during the forward, so the candidate still costs its payload bytes once here — strictly better than dying (which also takes ~2.8 neighbors with it), but bounded for the worst tail. The files-API payload cap remains the primary fix for that tail.
5. Guardrails: `admission_overflow_forwarded` / `admission_overflow_shed` counters, token-bucket cap on forward rate, fall back to admitting if the twin is unavailable.
