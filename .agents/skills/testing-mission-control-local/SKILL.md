...
- Revoke-keys dialog fixtures: label lookup means a masked API-key label beginning `sk-or-v1-` and containing `...`, not the key's human-readable name (see `app/admin-utils/revoke-keys/key-identifier.ts`). Seed a valid masked label and a known test-only secret hash to exercise label/secret/ID lookup on the same active key.
- When testing compromise filing, verify the Sentinel target and unchanged active-key/revocation state before confirming a revoke: filing is not revocation. For notice-history fixtures, use the exact incident-scoped `api_key:<id>:revocation:<uuid>` ledger key; include key-only/wrong-incident decoys and test a NULL holder, restoring temporary mutations afterward.
- To exercise a DB-backed history error (for example `/admin-utils/revoke-keys`) without source mocks, hold an `ACCESS EXCLUSIVE` transaction lock on only the history table, reload, and cancel the blocked history SELECTs via `pg_cancel_backend`. Dev-mode duplicate loads can mean cancelling one SELECT is insufficient: retain the lock until the UI visibly reaches the error state, then always `ROLLBACK` and reload to verify recovery. Filter `pg_stat_activity` by the exact table/query and `wait_event_type='Lock'`; never cancel unrelated sessions.
